...
from .deform_object_data import DeformObjectData

if TYPE_CHECKING:
    from .deform_object_cfg import DeformObjectCfg


class DeformObject(AssetBase):
    ...
    def __init__(self, cfg: DeformObjectCfg):
    ...
    @property
    def data(self) -> DeformObjectData:
        return self._data
    ...
    @property
    def root_physx_view(self) -> physx.DeformBodyView:
        """Deform body view for the asset (PhysX).
    ...
    @property
    def body_physx_view(self) -> physx.BodyView:
    ...
    def _initialize_impl(self):
        ...
        root_prims = sim_utils.get_all_matching_child_prims(
            template_prim_path, predicate=lambda prim: prim.HasAPI(UsdPhysics.DeformBodyAPI) #FIXME
        )
        if len(root_prims) != 1:
            raise RuntimeError(
        ...
        # -- object view
        self._root_physx_view = self._physics_sim_view.create_deform_body_view(root_prim_path_expr.replace(".*", "*")) #FIXME
        self._body_physx_view = self._root_physx_view
        # log information about the articulation
        carb.log_info(f"Deformable body initialized at: {self.cfg.prim_path} with root '{root_prim_path_expr}'.")
